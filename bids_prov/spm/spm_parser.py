...
def group_lines(lines: list) -> Dict[str, list]:
    ...
    # matlabbatch{3}.spm.stats.con.spmmat(1) = cfg_dep('Model estimation: SPM.mat File', substruct('.','val', '{}',
    # {2}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','spmmat'));
    ...
    for i, (common_prefix_act, end_line_list) in enumerate(task_groups.items()):
        ...
        for end_line in end_line_list:
            ...
            if not conf.has_parameter(left) and re.search(conf.PATH_REGEX, right) and re.search(conf.FILE_REGEX, right):
                ...
                # check if the line call cfg_dep and retrieve the first parameter retrieve all digits between
                # parenthesis
                ...
                if dependency is not None:
                    ...
                    closest_activity = find_closest_activity(records["Activities"], right)
                    ...
                    if closest_activity["Used"]:
                        ...
                        input_entity = find_entity_from_id(id_closest, records["Entities"])
...
def spm_to_bids_prov(filename: str, context_url=CONTEXT_URL, output_file=None, spm_ver="SPM12r7224", verbose=False,
    ...
    # Post-processing
    for activity in records["Activities"]:  # Remove each activity number from the activity labels
        activity["Label"] = re.sub(r'._\d+$', '', activity["Label"])

    compute_sha_256_entity(records["Entities"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_file", type=str, default="./examples/spm_default/batch.jsonld",
    ...
    # > python -m   bids_prov.spm.ls spm_parser --input_file ./nidm-examples/spm_2_t_test/batch.m --output_file
    # ./spm_2_t_test.jsonld
